...
struct gpio_init_entry Gpio_LowPower_Table[] = {
	{LED_GPIO_PORT, 		{LED_PIN, 			GPIO_MODE_ANALOG,		GPIO_NOPULL, 	GPIO_SPEED_LOW}		},
	{USART3_GPIO_PORT, 		{USART3_TX_PIN, 	GPIO_MODE_ANALOG,		GPIO_NOPULL, 	GPIO_SPEED_LOW}		},
	{USART3_GPIO_PORT, 		{USART3_RX_PIN, 	GPIO_MODE_ANALOG,		GPIO_PULLUP, 	GPIO_SPEED_LOW}		},
	{USART2_GPIO_PORT, 		{USART2_TX_PIN, 	GPIO_MODE_ANALOG,		GPIO_NOPULL, 	GPIO_SPEED_LOW}		},
	{USART2_GPIO_PORT, 		{USART2_RX_PIN, 	GPIO_MODE_ANALOG,		GPIO_PULLUP, 	GPIO_SPEED_LOW}		},
	{USART1_GPIO_PORT, 		{USART1_TX_PIN, 	GPIO_MODE_ANALOG,		GPIO_NOPULL, 	GPIO_SPEED_LOW}		},
	{USART1_GPIO_PORT, 		{USART1_RX_PIN, 	GPIO_MODE_ANALOG,		GPIO_PULLUP, 	GPIO_SPEED_LOW}		},
	{GSM_NETLIGHT_GPIO_PORT,{GSM_NETLIGHT_PIN,	GPIO_MODE_ANALOG,		GPIO_NOPULL,	GPIO_SPEED_LOW}		},
	{GSM_PWRKEY_GPIO_PORT,	{GSM_PWRKEY_PIN,	GPIO_MODE_ANALOG,		GPIO_NOPULL,	GPIO_SPEED_LOW }	},
	{USBDP_GPIO_PORT, 		{USBDP_PIN, 		GPIO_MODE_OUTPUT_PP,	GPIO_NOPULL, 	GPIO_SPEED_HIGH} 	}
};
...
void Board_Init(void)
{
	...
	//NVIC_EnableIRQ(RTC_IRQn);  /* Don't enable if only RTC Alarm interrupt is needed (redundant) */
...
#if !USB_ENABLED
	HAL_GPIO_WritePin(USBDP_GPIO_PORT, USBDP_PIN, GPIO_PIN_SET);
#endif
...
{
	GPIO_PinState 	charge_pin = HAL_GPIO_ReadPin(BAT_CHRG_GPIO_PORT, BAT_CHRG_PIN);
	return (charge_pin == GPIO_PIN_RESET) ? true : false;
}

void GPIO_Set_LowPower(void)
{
	uint32_t index;
	for(index = 0; index < sizeof(Gpio_LowPower_Table)/sizeof(Gpio_LowPower_Table[0]); index++)
	{
		HAL_GPIO_Init(Gpio_LowPower_Table[index].Port, &Gpio_LowPower_Table[index].Init);
	}
	/* USB D+ Pulled up to VCC, so set D+ to HIGH state to stop current draw */
	HAL_GPIO_WritePin(USBDP_GPIO_PORT, USBDP_PIN, GPIO_PIN_SET);
}
